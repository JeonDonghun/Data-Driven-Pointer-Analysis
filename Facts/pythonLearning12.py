...
def calClauseNum(a,partFormula,notD0Formula):
  ...
  os.system('cp heuristic.logic ../doop/logic/3-object-sensitive+3-heap/heuristic.logic')
  os.system('./run -jre1.6 3-object-sensitive+3-heap jars/dacapo/luindex.jar')
  os.system('bloxbatch -db ../doop/last-analysis -query D3 | sort > sig.facts')
  f = open("sig.facts","r")
...
def oneMimimalLearning (backUpFormula,startFormula,bestCost,bestPrecision,cmd,dooppath,changed,notD0Formula,D2Formula):
  ...
  os.system('bloxbatch -db ../doop/last-analysis -query Stats:Simple:PotentiallyFailingCast | sort > ChangedAlarm_Numluindex.facts')
  ...
  if(analysisTime == 'timeout'):
    ...
    os.system('cp backupantlr.facts ChangedAlarm_Numantlr.facts') 
    return backUpFormula,10000
  os.system('bloxbatch -db ../doop/last-analysis -query Stats:Simple:PotentiallyFailingCast | sort > ChangedAlarm_Numlusearch.facts')
  checkTime = checkTime+float(analysisTime)
  ...
  if(analysisTime == 'timeout'):
    ...
    os.system('cp backupantlr.facts ChangedAlarm_Numantlr.facts') 
    return backUpFormula,10000
  os.system('bloxbatch -db ../doop/last-analysis -query Stats:Simple:PotentiallyFailingCast | sort > ChangedAlarm_Numpmd.facts')
  checkTime = checkTime+float(analysisTime)
  ...
  if(analysisTime == 'timeout'):
    ...
    os.system('cp backupantlr.facts ChangedAlarm_Numantlr.facts') 
    return backUpFormula,10000
  os.system('bloxbatch -db ../doop/last-analysis -query Stats:Simple:PotentiallyFailingCast | sort > ChangedAlarm_Numantlr.facts')
  checkTime = checkTime+float(analysisTime)
  ...
  if(changedAlarmNum < bestPrecision*1.021 ):#alarmNum +1 < bestPrecision
    print("changed")
...
def makheuristic(D2Formula,startFormula,notD0Formula):
  ...
  f = open("heuristic.logic","w")
  f.write("D3(?meth) -> MethodSignatureRef(?meth).\n")
  f.write("D1(?meth) -> MethodSignatureRef(?meth).\n")
  f.write("D0(?meth) -> MethodSignatureRef(?meth).\n\n\n")
...
def makeRecentHeuristic(startFormula,notD0Formula):
  ...
  fa = open("recentHeuristic0123D3.logic","w")
  ...
  fa = open("recentHeuristic0123D0.logic","w")
...
os.system('cp ../doop/logic/depth1and2.logic  ../doop/logic/context-sensitive.logic')
...
dooppath = '../doop'
analysisTime = runDoop (cmd,dooppath)
os.system('bloxbatch -db ../doop/last-analysis -query Stats:Simple:PotentiallyFailingCast | sort > luindexAlarm_Num.facts')
...
cmd = 'run -jre1.6 3-object-sensitive+3-heap jars/dacapo/lusearch.jar'
analysisTime = runDoop (cmd,dooppath)
os.system('bloxbatch -db ../doop/last-analysis -query Stats:Simple:PotentiallyFailingCast | sort > lusearchAlarm_Num.facts')

timeRepository = timeRepository + float(analysisTime)

cmd = 'run -jre1.6 3-object-sensitive+3-heap jars/dacapo/pmd.jar'
analysisTime = runDoop (cmd,dooppath)
os.system('bloxbatch -db ../doop/last-analysis -query Stats:Simple:PotentiallyFailingCast | sort > pmdAlarm_Num.facts')

timeRepository = timeRepository + float(analysisTime)

cmd = 'run -jre1.6 3-object-sensitive+3-heap jars/dacapo/antlr.jar'
analysisTime = runDoop (cmd,dooppath)
os.system('bloxbatch -db ../doop/last-analysis -query Stats:Simple:PotentiallyFailingCast | sort > antlrAlarm_Num.facts')
...
for f in range(0,len(Fset)):
      ...
      if(bestPrecision*1.021 > precisionCheck):

...
function create-database()
{
    ...
    do
        ...
        if [[ -e $cachefacts ]] && flag-isset cache; then
            ...
            echo "" > $cachefacts/BreakpointStmtInMeth.facts
            echo "" > $cachefacts/RetStmtInMeth.facts
            echo "" > $cachefacts/NopStmtInMeth.facts

            #cp $DOOP_HOME/tmp/facts/MethodDeclaration.facts $DOOP_HOME/methods/
